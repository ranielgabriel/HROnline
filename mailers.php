...
$ref = "i9802080ed";/* $_POST['ref']; */
$concat = "";
//$email = $_POST['email'];
$email = "arvynn.c.banez@gmail.com";
...
$result = $conn->query($select_ref);
	while ($row = $result->fetch_assoc()) {
	 ...
	 if($row["email_status"]==1){
		if($row["hr_email"]==0){
			$email='iamjovyvillafuerte@gmail.com';
			$concat= "&&iNt3RvIs3w3r";
...
$mail->SMTPSecure = 'tls';
$mail->SMTPAuth = true;
$mail->Username = "iamjovyvillafuerte@gmail.com";
$mail->Password = "awesomeJV222";
...
if (!$mail->send()) {
	...
	/* if(isset($_POST["hr"])){
		if($_POST["hr"]=="yes"){ */
			...
			$mail->setFrom('phrecruitment@andersongroup.uk.com','Anderson Group PH');
			$mail->addReplyTo('no-reply@example.com');
			$mail->addAddress("iamjovyvillafuerte@gmail.com");
